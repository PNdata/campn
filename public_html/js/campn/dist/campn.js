...
UI.Dialog = function (title, $content, options, onValid, onCancel) {
  ...
  this.enableButton = function (button, enable) {
    if (enable)
      button.removeAttr('disabled').removeClass('k-state-disabled');
    else
      button.attr('disabled', 'disabled').addClass('k-state-disabled');
  }
...
App.login = function (auth, encrypted, done, fail) {
  App.Server.Auth.login(
          ...
          function (data) {
            ...
            if (encrypted)
              App.account.auth.str = auth;
            else
              App.account.auth = auth;
                        ...
                        $('#query-list').sortable({
                          ...
                          update: function (e, ui) {
                            ...
                            if (App.session.computeOnTheFly()) {
                              ...
                              if (queryToCompute != null)
                                App.Controller.Query.computeFrom(queryToCompute, function () {}, function () {});
                            }
                          }
                        });
                        App.UI.loaded();
...
App.UI.shortcuts = [
  {value: 'save', text: 'Enregistrer', icon: 'disk', event: function () {
      App.Controller.Session.save();
    }},
  {value: 'open', text: 'Ouvrir', icon: 'folder-open', event: function () {
      App.Controller.Session.open();
    }},
  {value: 'new', text: 'Nouveau', icon: 'file-2', event: function () {
      App.Controller.Session.confirmNew();
    }}
];
...
App.UI.menu = {
  file: {isDefault: true, text: "Fichier", items: [
      {enabled: true, value: "new", text: "Nouveau", icon: "file-4", event: function () {
          App.Controller.Session.confirmNew();
        }},
      {enabled: true, value: "open", text: "Ouvrir", icon: "folder-open", event: function () {
          App.Controller.Session.open();
        }},
      {enabled: true, value: "save", text: "Enregistrer", icon: "upload-3", event: function () {
          App.Controller.Session.save();
        }},
      {enabled: true, value: "saveas", text: "Enregistrer sous", icon: "upload-3", event: function () {
          App.Controller.Session.openSaveAs();
        }},
      {enabled: true, value: "print", text: "Imprimer", icon: "print", event: function () {
          App.Controller.Session.openPrint();
        }},
      {enabled: true, value: "exit", text: "Quitter", icon: "switch", event: function () {
          App.Controller.Session.confirmExit();
        }},
    ]}/*,
  ...
  query: {text: "Requêtage", items: [
      {enabled: true, value: "add", text: "Ajouter", icon: "plus", event: function () {
          App.Controller.Query.dialog();
        }},
      ...
      {enabled: true, value: "report", text: "Rapport Excel", icon: "file-excel", event: function () {
          App.Controller.Session.report();
        }},
      {enabled: true, value: 'createNodeModel', text: 'Modèles de noeud', icon: 'file', event: function () {
          App.Controller.Node.openCreateNodeModel();
        }}
    ]},
  tools: {text: "Outils", items: [
      {enabled: true, value: "database", text: "Dictionnaire de données", icon: "books", event: function () {
          App.DataModel.dialog('Base de données', {commands: {cancel: {display: false}}});
        }},
      {enabled: true, value: "settings", text: "Préférences", icon: "settings", event: function () {
          App.Controller.Session.openSettings();
        }}
    ]},
  extraction: {
    text: "Extraction",
    items: [
      {enabled: true, value: "extract", text: "Extraire", icon: "upload-3", event: function () {
          App.Controller.Session.openExport();
        }},
      {enabled: true, value: "list", text: "Historique", icon: "history", event: function () {
          App.Controller.Session.openExtractionsList();
        }},
    ]
  }
  ...
  help: {text: "?", items: [
      ...
      {enabled: true, value: "about", text: "A propos de", icon: "info", event: function () {
          App.openAbout()
        }},
...
App.UI.buildMenu = function () {
  ...
  $.each(App.UI.menu, function (i, item) {
    ...
    if (item.isDefault)
      $submenu.removeClass('hide').addClass('flex');
    $item.click(function () {
...
App.DataModel = {
  ...
  dialog: function (title, callback) {
    var that = this;
    var dialog = new UI.Dialog('Modèle de données', this.tree(), {width: 600}, function () {});
    dialog.title(title);
    dialog.open();
...
App.Server.Criterion = {
  file: function (data, done, fail) {
    ...
    return $.ajax({
      url: 'http://localhost/ComptageDirectFichiersServer/criterion/file',
      type: 'POST',
      processData: false,
      contentType: false,
      dataType: 'json',
...
App.Controller.Session.openExport = function () {
  ...
  settings.get("fields").add([
    {path: '@id_foyer', label: 'ID foyer'},
    {path: '@civilite', label: 'Civilité'},
    {path: '@prenom', label: 'Prénom'},
    {path: '@nom', label: 'Nom'},
    {path: '@ligne_adr_2', label: 'Adresse zone 2'},
    {path: '@ligne_adr_3', label: 'Adresse zone 3'},
    {path: '@ligne_adr_4', label: 'Adresse zone 4'},
    {path: '@ligne_adr_5', label: 'Adresse zone 5'},
    {path: '@code_postal', label: 'Code postal'},
    {path: '@ville', label: 'Ville'},
    {path: '@type_habitat', label: 'Type habitat'},
    {path: '@code_insee', label: 'Code INSEE'},
    {path: '@code_iris', label: 'Code IRIS'}
  ]);
  ...
  $('<label for="extraction-name">Nom de l\'extraction</label>').appendTo($extractionName);
  ...
  $('<input type="text" id="extraction-filename" class="k-textbox"/>').appendTo($extractionFilename).on("change", function () {
    extraction.set("filename", $(this).val());
  });
  ...
  var $dataModel = dataModel.tree(false).attr('id', 'export-selection-vars').width('50%');
...
App.Controller.Query.openLoadModel = function (query) {
...
App.Controller.Query.dialog = function (query) {
  ...
  var $nameLabel = $('<label for="query-name">Code cible</label>');
...
App.Controller.Query.computeFrom = function (query, done, fail) {
  ...
  App.Controller.Query.compute(
          ...
          function (data) {
            if (query.hasNext()) {
              App.Controller.Query.computeFrom(query.next(), function () {
                done(data);
              }, function () {});
            } else if (query.excl()) {
              if (App.session.queries().length > 0) {
                App.Controller.Query.computeFrom(App.session.queries()[0], function () {
                  done(data);
                }, function () {});
              }
            }
...
App.Controller.Node.computeFrom = function (node, done, fail, progression) {
  ...
  App.Controller.Node.compute(
          ...
          function (data) {
            if (node.hasChildren()) {
              App.Controller.Node.computeFrom(node.first(), function () {
                done(data); /*progression();*/
              }, function () {}, progression);
            }
            if (node.hasNext()) {
              App.Controller.Node.computeFrom(node.next(), function () {
                done(data); /*progression(); */
              }, function () {}, progression);
            }
...
function Session() {
  ...
  this.computeOnTheFly = function (computeOnTheFly) {
    if (computeOnTheFly != null) {
      this._computeOnTheFly = computeOnTheFly;
      return this;
    } else {
      return this._computeOnTheFly;
    }
  }

  this.excludeQueries = function (excludeQueries) {
    if (excludeQueries != null) {
      this._excludeQueries = excludeQueries;
      return this;
    } else {
  ...
  this.exclQuery = function (exclQuery) {
    if (exclQuery != null) {
      this._exclQuery = exclQuery;
      return this;
    } else {
      return this._exclQuery;
    }
  }
  ...
  this.popcount = function () {
    return _.reduce(this.queries(), function (memo, value) {
      return memo + value.popcount()
    }, 0);
  };
...
function Query(data) {
  ...
  this.useExclusion = function (useExclusion) {
    if (useExclusion != null) {
      this._useExclusion = useExclusion;
      return this;
    } else {
      return this._useExclusion;
    }
  }
  ...
  this.commands = function () {
    ...
    return {
      ...
      load: function () {
        App.Controller.Query.openLoadModel(that);
      },
      edit: function () {
        App.Controller.Query.dialog(that);
      },
...
function Node(data) {
  ...
  this.toString = function () {
    ...
    var critStr = _.map(crits, function (c) {
      return c.toString();
    }).join(" et ");
  ...
  this.position = function () {
    ...
    if (!this.hasParent()) {
      ...
      if (this.parent().children().length == 1) { // si enfant unique => top du parent
        ...
        if (!this.previous().hasChildren())
          position.top = this.previous().$e().pos('top') + 180 + 10; // si le précédent n'a pas d'enfant => top précédent + 180 + 10                 
        else
          position.top = this.previous().lastOfBranch().$e().pos('top') + 180 + 10; // sinon => top dernier de la branche du précédent + 180 + 10
      } else {
      }
      ...
      if (!(that.query().nodes().indexOf(that) == 0 && name == 'remove')) {
        if (cmd.enabled) {
          var $button = $('<button id="node-command-' + name + '" title="' + cmd.tooltip + '" class="k-button node-command-' + name + '"><i class="icon-' + cmd.icon + '"></i></button>')
                  ...
                  .hover(function () {
                    $(this)/*.css('color', color.buttonOver )*/
                  },
                          function () {
                            $(this)/*.css('color', color.button     )*/
                          })
          $commands.append($button);
        }
      }
  ...
  this.commands = function () {
    ...
    return {
      copy: function () {},
      save: function () {
        App.Controller.Node.openSaveModel(that)
      },
      load: function () {
        App.Controller.Node.openLoadModel(that)
      },
      addCriterion: function () {
        App.Controller.Criterion.add(that);
      },
      edit: function () {
        App.Controller.Node.edit(that);
      },
      remove: function () {
        App.Controller.Node.delete(that);
      },
      addNode: function () {
        App.Controller.Node.add(that.query(), null, that);
      }
    };
  }
...
function Attribute(data) {
  ...
  // Propriétés autorisées
  this.allowedProperties = ['name', 'label', 'type', 'format', 'schema', 'path', 'dataType', 'linkCardinality', 'keyName', 'hugeData', 'enum'];
  ...
  this.linkCardinality = function (linkCardinality) {
    if (linkCardinality != null) {
      this._linkCardinality = linkCardinality;
      return this;
    } else {
      return this._linkCardinality;
    }
  }
